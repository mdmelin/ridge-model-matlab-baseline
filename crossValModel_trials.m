function [Vm, cBeta, cRidge, cMap, cMovie] =  crossValModel_trials(R, U, Vc, folds, frames)
% function to compute cross-validated R^2


Vm = zeros(size(Vc),'single'); %pre-allocate motor-reconstructed V
rng(1) % for reproducibility
trialIdx = randperm(size(Vc,2) / frames);
...
for iFolds = 1:folds
    ...
    if folds > 1
        ...
        if iFolds == 1
            [cRidge, cBeta{iFolds}] = ridgeMML(Vc(:,dataIdx)', R(dataIdx,:), true); %get beta weights and ridge penalty for task only model
        else
            [~, cBeta{iFolds}] = ridgeMML(Vc(:,dataIdx)', R(dataIdx,:), true, cRidge); %get beta weights for task only model. ridge value should be the same as in the first run.
        end
        
        Vm(:,~dataIdx) = (R(~dataIdx,:) * cBeta{iFolds})'; %predict remaining data
        
        if rem(iFolds,folds/5) == 0
            fprintf(1, 'Current fold is %d out of %d\n', iFolds, folds);
        end
    else
        [cRidge, cBeta{iFolds}] = ridgeMML(Vc', R, true); %get beta weights for task-only model.
        Vm = (R * cBeta{iFolds})'; %predict remaining data
